...
export function renderFlowCV(data, fontFamily, fontSize) {
      ...
      <div class="space-y-3">
        ${expList.map(exp => {
          ...
          <div class="entry-block space-y-0.5">
            ...
            <div class="text-[10px] text-slate-600 font-bold">${esc(comp)}${loc ? ` &mdash; <span class="text-slate-400 font-normal">${esc(loc)}</span>` : ''}</div>
            ${bulletList(b, 'text-[10px] text-slate-600')}
          </div>`;
        }).join('')}
      </div>
    </div>` : '';
      ...
      <div class="space-y-3">
        ${internshipList.map(item => {
          ...
          <div class="entry-block space-y-0.5">
            ...
            <div class="text-[10px] text-slate-600 font-bold">${esc(comp)}${loc ? ` &mdash; <span class="text-slate-400 font-normal">${esc(loc)}</span>` : ''}</div>
            ${bulletList(desc, 'text-[10px] text-slate-600')}
          </div>`;
        }).join('')}
      </div>
    </div>` : '';
      ...
      <div class="space-y-2">
        ${projList.map(proj => {
          ...
          <div class="entry-block space-y-0.5">
            ...
            ${bulletList(desc, 'text-[10px] text-slate-600 leading-normal')}
          </div>`;
        }).join('')}
      </div>
    </div>` : '';
      ...
      <div class="space-y-2">
        ${eduList.map(edu => {
          ...
          <div class="entry-block space-y-0.5">
            ...
            ${score ? `<div class="text-[10px] font-semibold text-slate-700">CGPA/Score: ${esc(score)}</div>` : ''}
          </div>`;
        }).join('')}
      </div>
    </div>` : '';
  ...
  <div class="p-8 bg-white text-[#2D3748] max-w-[800px] mx-auto text-left font-sans leading-normal">
    <div class="mb-5 pb-3 border-b border-slate-200">
      <h1 class="text-3xl font-black text-[#1A202C] tracking-tight">${name}</h1>
      ${headline ? `<div class="text-[11px] font-bold text-slate-500 uppercase tracking-wider mt-0.5">${esc(headline)}</div>` : ''}
