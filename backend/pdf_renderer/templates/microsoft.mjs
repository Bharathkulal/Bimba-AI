...
export function renderMicrosoft(data, fontFamily, fontSize) {
  ...
  const skills = skillCategories.length ? `
    <div class="mb-4">
      <h3 class="${sectionH3}">Core Competencies &amp; Technical Skills</h3>
  ...
  const personalSkills = personalSkillsList.length ? `
    <div class="mb-4">
      <h3 class="${sectionH3}">Personal / Professional Skills</h3>
    ...
    <div class="mb-4">
      ...
      <div class="space-y-3">
        ${expList.map(exp => {
          ...
          return `
          <div class="entry-block space-y-0.5 text-left">
            <div class="flex justify-between items-baseline text-[11px] font-bold text-slate-850">
              <span>${esc(pos)} &mdash; ${esc(comp)}</span>
              <span class="font-medium text-[10px] text-slate-500">${esc(dur)}</span>
            </div>
            ${loc ? `<div class="text-[10px] text-slate-500 font-medium italic">${esc(loc)}</div>` : ''}
  ...
  const internships = internshipList.length ? `
    <div class="mb-4">
      <h3 class="${sectionH3}">Internship Experience</h3>
      <div class="space-y-3">
        ${internshipList.map(item => {
          ...
          <div class="entry-block space-y-0.5 text-left">
            <div class="flex justify-between items-baseline text-[11px] font-bold text-slate-850">
              <span>${esc(role)} &mdash; ${esc(comp)}</span>
              <span class="font-medium text-[10px] text-slate-500">${esc(dur)}</span>
            </div>
            ${loc ? `<div class="text-[10px] text-slate-500 font-medium italic">${esc(loc)}</div>` : ''}
        ...
        ${eduList.map(edu => {
          ...
          <div class="entry-block space-y-0.5">
            <div class="flex justify-between items-baseline text-[11px] font-bold text-slate-850">
              ...
              <span class="font-medium text-[10px] text-slate-500">${esc(yr)}</span>
            </div>
            <div class="flex justify-between items-baseline text-[10px] text-slate-600">
  ...
  <div class="p-8 bg-white text-[#111111] max-w-[800px] mx-auto text-left font-sans leading-normal">
    <div class="border-b-4 border-slate-900 pb-3 mb-5">
      ...
      <div class="text-[10.5px] text-slate-500 font-bold tracking-wide mt-1 flex flex-wrap gap-2.5">
