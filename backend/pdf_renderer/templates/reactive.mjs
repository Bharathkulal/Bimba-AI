...
export function renderReactive(data, fontFamily, fontSize) {
  ...
  const name = esc(p.name || 'Candidate Name');
        ...
        ${expList.map(exp => {
          ...
          <div class="entry-block space-y-1">
            <div class="flex justify-between items-baseline text-[11px] font-bold text-slate-800">
              <span>${esc(pos)} at ${esc(comp)}</span>
              <span class="font-semibold text-[9.5px] text-slate-500">${esc(dur)}</span>
            </div>
            ${loc ? `<div class="text-[10px] text-slate-500 italic">${esc(loc)}</div>` : ''}
    ...
    <div class="mb-4">
      ...
      <div class="space-y-3">
        ${internshipList.map(item => {
          ...
          <div class="entry-block space-y-1">
            <div class="flex justify-between items-baseline text-[11px] font-bold text-slate-800">
              <span>${esc(role)} at ${esc(comp)}</span>
              <span class="font-semibold text-[9.5px] text-slate-500">${esc(dur)}</span>
            </div>
            ${loc ? `<div class="text-[10px] text-slate-500 italic">${esc(loc)}</div>` : ''}
          ...
          <div class="entry-block space-y-1">
            <div class="flex justify-between items-baseline text-[11px] font-bold text-slate-800">
              ...
              <div class="flex items-center gap-2">
                ${url ? `<a href="${url.startsWith('http') ? esc(url) : 'https://' + esc(url)}" target="_blank" class="text-[9.5px] text-indigo-600 font-semibold hover:underline">Link</a>` : ''}
                ${dur ? `<span class="text-[9.5px] text-slate-500 font-normal">${esc(dur)}</span>` : ''}
              </div>
            </div>
            ${bulletList(desc, 'text-[10.5px] text-slate-650')}
          </div>`;
      ...
      <div class="space-y-2">
        ${eduList.map(edu => {
          ...
          <div class="entry-block space-y-0.5">
            ...
            ${score ? `<div class="text-[10px] font-semibold text-indigo-600">CGPA/Score: ${esc(score)}</div>` : ''}
          </div>`;
        }).join('')}
      </div>
    </div>` : '';
  ...
  <div class="p-8 bg-white text-[#1E293B] max-w-[800px] mx-auto text-left font-sans leading-normal">
    <div class="mb-5 flex justify-between items-end border-b-2 border-indigo-600 pb-3">
      <div>
        <h1 class="text-3xl font-black text-slate-900 tracking-tight">${name}</h1>
        ${headline ? `<p class="text-[11px] text-indigo-600 font-bold uppercase tracking-wider mt-0.5">${esc(headline)}</p>` : ''}
      </div>
