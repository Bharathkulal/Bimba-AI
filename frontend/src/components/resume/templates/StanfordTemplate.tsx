...
export const StanfordTemplate: React.FC<TemplateProps> = ({ 
  data, 
  ...
  return (
    <div 
      className="p-8 bg-white text-[#1E293B] max-w-[800px] mx-auto text-left leading-relaxed shadow-sm min-h-[1050px]"
      style={{ fontFamily: fontFamily || 'Georgia, "Times New Roman", serif', fontSize }}
    >
      {/* Header */}
      <div className="text-center mb-5 border-b-2 border-red-800 pb-4">
        <h1 className="text-3xl font-extrabold tracking-tight text-red-800 uppercase">{pInfo.name || ''}</h1>
              ...
              return (
                <div key={idx} className="space-y-0.5">
                  <div className="flex justify-between items-baseline text-[11px] font-bold text-slate-800">
                    ...
                    <span className="font-medium text-[10.5px] text-slate-500">{yr}</span>
                  </div>
                  <div className="flex justify-between items-baseline text-[10.5px] text-slate-600">
                    ...
                    {score && <span className="font-semibold text-red-800">CGPA/Score: {score}</span>}
                  </div>
                </div>
              );
            })}
      ...
      {data.experience && data.experience.length > 0 && (
        <div className="mb-4">
          <h3 className={sectionH3}>Academic &amp; Professional Experience</h3>
              ...
              return (
                <div key={idx} className="space-y-1">
                  <div className="flex justify-between items-baseline text-[11.5px] font-bold text-slate-800">
                    ...
                    <span className="font-semibold text-[10px] text-slate-500">{dur}</span>
                  </div>
                  <div className="text-[10.5px] text-slate-600 italic font-medium">
                    <span>{exp.company || exp.organization}</span>
                    {exp.location && <span className="not-italic text-slate-500 ml-1">({exp.location})</span>}
                  </div>
                  {bullets.length > 0 && (
              ...
              return (
                <div key={idx} className="space-y-1">
                  ...
                  <div className="text-[10.5px] text-slate-600 italic font-medium">
                    ...
                    {item.location && <span className="not-italic text-slate-500 ml-1">({item.location})</span>}
                  </div>
                  {bullets.length > 0 && (
                    <ul className="list-disc pl-4 space-y-0.5 text-[10.5px] text-slate-650">
                      {bullets.map((b, i) => <li key={i}>{b}</li>)}
                  ...
                  <div className="flex justify-between items-baseline text-[11px] font-bold text-slate-800">
                    ...
                    <div className="flex items-center gap-2">
                      {(proj.url || proj.link || proj.github) && (
                        <a href={proj.url || proj.link || proj.github} target="_blank" rel="noreferrer" className="text-[9.5px] text-red-800 font-semibold hover:underline">Link</a>
                      )}
          ...
          <div className="space-y-2">
            {data.certifications.map((cert: any, idx: number) => (
              <div key={idx} className="flex justify-between items-baseline text-[11px] font-bold text-slate-800">
                ...
                <span className="font-medium text-[10.5px] text-slate-500">{cert.issue_date || cert.year || cert.date}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {data.publications && data.publications.length > 0 && (
        <div className="mb-4">
          <h3 className={sectionH3}>Publications &amp; Research</h3>
      ...
      {data.achievements && data.achievements.length > 0 && (
        <div className="mb-4">
          <h3 className={sectionH3}>Honors &amp; Awards</h3>
          ...
          <div className="space-y-2 text-[11px] text-slate-700">
            {data.leadership.map((item: any, idx: number) => (
              <div key={idx}>
                ...
                {item.description && <p className="text-[10.5px] text-slate-650 mt-0.5">{item.description}</p>}
              </div>
            ))}
          </div>
        </div>
