...
export const FlowCVTemplate: React.FC<TemplateProps> = ({ 
  data, 
  ...
  const sectionH3 = 'text-xs font-extrabold uppercase tracking-widest text-[#4A5568] border-b border-slate-200 pb-1 mb-2 mt-4';

  return (
    <div 
      className="p-8 bg-white text-[#2D3748] max-w-[800px] mx-auto text-left font-sans leading-normal shadow-sm min-h-[1050px]"
      style={{ fontFamily, fontSize }}
    >
      {/* Header */}
      <div className="mb-5 pb-3 border-b border-slate-200">
        <h1 className="text-3xl font-black text-[#1A202C] tracking-tight uppercase">{pInfo.name || ''}</h1>
      ...
      {data.summary && (
        <div className="mb-4">
          <h3 className={sectionH3}>About</h3>
              ...
              return (
                <div key={idx} className="space-y-0.5">
                  <div className="flex justify-between items-baseline text-[11px] font-bold text-slate-800">
                    ...
                    <span className="font-semibold text-[9.5px] text-slate-400">{dur}</span>
                  </div>
                ...
                <div key={idx} className="space-y-0.5">
                  <div className="flex justify-between items-baseline text-[11px] font-bold text-slate-800">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {dur && <span className="font-normal text-[9.5px] text-slate-400">{dur}</span>}
                    </div>
                  </div>
                  {tech && <div className="text-[9.5px] text-slate-500 font-medium">({tech})</div>}
                  {bullets.length > 0 && (
                    <ul className="list-disc pl-4 space-y-0.5 text-[10px] text-slate-600 leading-normal">
                      {bullets.map((b, i) => <li key={i}>{b}</li>)}
                    </ul>
                  )}
                </div>
      ...
      {skillCategories.length > 0 && (
        <div className="mb-4">
          ...
          <div className="space-y-1 text-[10.5px] text-slate-600 leading-relaxed">
      ...
      {personalSkills.length > 0 && (
        <div className="mb-4">
          ...
          <p className="text-[10.5px] text-slate-600 leading-relaxed">{personalSkills.join(', ')}</p>
        </div>
      )}
            ...
            {data.education.map((edu: any, idx: number) => {
              ...
              return (
                <div key={idx} className="space-y-0.5">
                  ...
                  {score && <div className="text-[10px] font-semibold text-slate-700">CGPA/Score: {score}</div>}
                </div>
              );
            })}
          </div>
      ...
      {hobbies.length > 0 && (
        <div className="mb-4">
          ...
          <p className="text-[10.5px] text-slate-600 leading-relaxed">{hobbies.join(', ')}</p>
        </div>
      )}
