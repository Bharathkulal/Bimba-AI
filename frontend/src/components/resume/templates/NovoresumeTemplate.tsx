...
export const NovoresumeTemplate: React.FC<TemplateProps> = ({ 
  data, 
  ...
  return (
    <div 
      className="p-8 bg-white text-[#2C3E50] max-w-[800px] mx-auto text-left font-sans leading-normal shadow-sm min-h-[1050px]"
    ...
    >
      ...
      <div className="border-t-4 border-[#3498DB] pt-4 mb-5 text-center">
        ...
        {(pInfo.title || data.target_role) && (
          <div className="text-[11px] font-bold text-[#3498DB] uppercase tracking-wider mt-0.5">
            {pInfo.title || data.target_role}
          </div>
        )}
        <div className="text-[10px] text-slate-600 font-semibold mt-2 flex flex-wrap justify-center gap-2">
            ...
            {data.experience.map((exp: any, idx: number) => {
              ...
              return (
                <div key={idx} className="space-y-0.5">
                  ...
                  <div className="text-[10px] text-[#3498DB] font-semibold">
                    {exp.company || exp.organization}
                    {exp.location && <span className="text-slate-500 font-normal ml-1">({exp.location})</span>}
                  </div>
                  {bullets.length > 0 && (
                    <ul className="list-disc pl-4 space-y-0.5 text-[10px] text-slate-650">
              ...
              return (
                <div key={idx} className="space-y-0.5">
                  ...
                  <div className="text-[10px] text-[#3498DB] font-semibold">
                    ...
                    {item.location && <span className="text-slate-500 font-normal ml-1">({item.location})</span>}
                  </div>
                  {bullets.length > 0 && (
                    <ul className="list-disc pl-4 space-y-0.5 text-[10px] text-slate-650">
                      {bullets.map((b, i) => <li key={i}>{b}</li>)}
                  ...
                  <div className="flex justify-between items-baseline text-[11px] font-bold text-slate-800">
                    ...
                    <div className="flex items-center gap-2">
                      {(proj.url || proj.link || proj.github) && (
                        <a href={proj.url || proj.link || proj.github} target="_blank" rel="noreferrer" className="text-[9.5px] text-[#3498DB] font-semibold hover:underline">Link</a>
                      )}
                      {dur && <span className="font-normal text-[9.5px] text-slate-500">{dur}</span>}
                    </div>
                  </div>
            ...
            {data.education.map((edu: any, idx: number) => {
              ...
              return (
                <div key={idx} className="space-y-0.5">
                  ...
                  <div className="flex justify-between items-baseline text-[10px] text-[#3498DB] font-semibold">
                    <span>{edu.institution || edu.school || edu.university}{edu.location ? `, ${edu.location}` : ''}</span>
                    {score && <span className="text-slate-700 font-normal">CGPA/Score: {score}</span>}
                  </div>
                </div>
              );
            })}
